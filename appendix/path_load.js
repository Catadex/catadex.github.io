(function() {
    document.title = "Loading Catadex";
    
    inStyle(`@keyframes loading{0%{transform:rotate(0) scale(.8)}50%{transform:rotate(180deg) scale(1.1)}100%{transform:rotate(359deg) scale(.8)}}:root{--accent-half:#6999f2;--text-color:#cfcfcf;--background:#161616}*{scrollbar-width:thin!important;scrollbar-color:var(--accent-half) transparent!important;box-sizing:border-box;transition-duration:.1s;transition-timing-function:ease-in-out;outline:0}::-webkit-scrollbar{width:7px!important;height:7px!important}::-webkit-scrollbar-thumb{background:var(--accent-half)!important}::-webkit-scrollbar-track{background:rgba(0,0,0,0)!important}::-moz-selection{color:var(--background);background:var(--accent-half)}::selection{color:var(--background);background:var(--accent-half)}body,html{height:100%;width:100%;line-height:1.3}html{font-family:"Radio Canada",sans-serif;font-size:18px}@media screen and (max-width:1200px){html{font-size:17px}}@media screen and (max-width:992px){html{font-size:16px}}@media screen and (max-width:768px){html{font-size:15px}}@media screen and (max-width:600px){html{font-size:14px}}body{color:var(--text-color);background:var(--background);padding:0;margin:0}.fa{-webkit-tap-highlight-color:transparent;-webkit-touch-callout:none;-webkit-user-select:none;-moz-user-select:none;user-select:none}.fa.splash{animation:loading 2s infinite linear;fill:var(--text-color);width:100%;max-width:5rem}content{height:100%;width:100%;z-index:1}.container{background:0 0;width:100%;padding:1rem 2rem;color:var(--text-color);word-wrap:break-word}@media screen and (max-width:540px){.container{padding:1rem 1.5rem}}.flex{display:flex}.flex.flex-center-v{align-items:center}.flex.flex-center-h{justify-content:center}.flex.flex-column{flex-direction:column}.flex.flex-row{flex-direction:row}.flex .flex-grow{flex:1 0 auto}main.load{position:relative;row-gap:2.5rem}main.load>span{-moz-column-gap:.5rem;column-gap:.5rem}main.load percent{position:fixed;bottom:1.75rem}main.load loadbar{position:fixed;left:0;bottom:0;display:block;height:.75rem;width:var(--loadbar-percent);background:var(--accent-half);transition:width .1s linear}`,
            "loadCSS");
...
const earl = "<scrolltop></scrolltop>";
...
function checkLocal() {
    ...
    localInt = countLocal("version") +
               ...
               countLocal("alltherollCSS") +
               countLocal("cascadiacodeCSS") +
               countLocal("radiocanadaCSS") +
    
               countLocal("baseJS") +
               countLocal("databaseJS") +
               countLocal("gradientanimationJS") +
               countLocal("homesubmitJS") +
               countLocal("nextbuttonsJS") +
               countLocal("searchbehaviorJS") +
               countLocal("searchfilterJS") +
               countLocal("thumbfilterJS") +
               countLocal("indbJS") +
               countLocal("clickactionJS") +
               
               countLocal("fable_home") +
               countLocal("fable_index") +
...
// Fetch various data
fetchThis(xJSON+"?"+dataver).then(function(value) {
    localforage.setItem("index_" + xNAME, value).then(function() {
        localStorage.version = version;
        localStorage.setItem("dataver_" + xNAME, dataver);
    });
    /*
    try {
        JSON.parse(value);
        const gotPako = pako.deflate(value, {level: 9});
        localforage.setItem("index_" + xNAME, gotPako.toString()).then(function() {
            localStorage.version = version;
            localStorage.setItem("dataver_" + xNAME, dataver);
        });
    } catch(e) {
        let txt = pako.inflate(JSON.parse("[" + value + "]")),
            txt2 = new TextDecoder().decode(txt);
        localforage.setItem("index_" + xNAME, txt2).then(function() {
            localStorage.version = version;
            localStorage.setItem("dataver_" + xNAME, dataver);
        });
    }
    */
    ...
    addScript("appendix/fable_index.min.js"+datever, true);
...
fetchRest("css","alltheroll");
fetchRest("css","cascadiacode");
fetchRest("css","radiocanada");

fetchRest("js","base");
fetchRest("js","database");
fetchRest("js","gradientanimation");
fetchRest("js","homesubmit");
fetchRest("js","nextbuttons");
fetchRest("js","searchbehavior");
fetchRest("js","searchfilter");
fetchRest("js","thumbfilter");
fetchRest("js","indb");
fetchRest("js","clickaction");
